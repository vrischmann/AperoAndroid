...
internal class SecretBox(key: ByteArray) {
    ...
    fun seal(message: ByteArray, nonce: ByteArray): ByteArray {
        ...
        // encrypt plaintext
        val out = ByteArray(message.size + poly1305.macSize)
        xSalsa20.processBytes(message, 0, message.size, out, poly1305.macSize)

        // hash ciphertext and prepend MAC to ciphertext
        poly1305.init(KeyParameter(subKey))
        poly1305.update(out, poly1305.macSize, message.size)
